...
describe('budget - validateBudgetExists', () => {
    ...
    it('should call next middleware if expense exists', async () => {
        (Budget.findByPk as jest.Mock).mockRejectedValue(new Error('Hubo un error'))
        const req = createRequest({
            params: {
                budgetId: 1
            }
    ...
    it('should proced to next middleware if budget exists', async () => {
        (Budget.findByPk as jest.Mock).mockResolvedValue(budgets[0]) // Vamos a hacer que el presupuesto exista
